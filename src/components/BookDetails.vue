<template>
  <base-dialog v-if="book != null">
    ...
    <template #read>
      ...
      <router-link
        class="read"
        v-else-if="book.savedPage == null"
        :to="`/${this.$route.params.bookId}/read/1`"
        >Read</router-link
      >
      <router-link
        v-else
        class="read"
        :to="`/${this.$route.params.bookId}/read/` + book.savedPage"
        >Continue</router-link
      >
      <router-link
        class="otherOptions"
        :to="`/details/${this.$route.params.bookId}/edit`"
        >Edit</router-link
      >
      <router-link class="otherOptions" to="/" @click="deleteBook(book.id)"
        >Delete</router-link
      >
    </template>
    <template #chapters>
      <base-chapters v-for="(chapter, index) in book.chapters" :key="chapter">
        ...
        <router-link
          class="chapterLink"
          :to="`/${this.$route.params.bookId}/read/` + chapter.page"
        >
          {{ chapter.name }}
        </router-link>
